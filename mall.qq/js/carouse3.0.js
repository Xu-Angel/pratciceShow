 ...
 */
/*负责京东首页第四个轮播  */
function carouselThree(container, arrowContainer, pointStyle, time, speed) {
    ...
    ul.style.height = liS.length * 100 + '%';//图片总宽度
    /*判断是否需要圆点*/
    if (pointStyle != undefined) {
        var point =document.getElementsByClassName('fourPoint')[0];//小圆点ul
        ...
        */
        for (var j = 0; j < points.length; j++) {
            ...
            points[j].onmouseover = function () {
                ...
                index = this.index;
                caranimateFour(ul, -index * container.offsetHeight, speed);
                light();
            }
    ...
    */
    ...
    function rightPlay() {
        ...
        caranimateFour(ul, -index * container.offsetWidth, speed);
    ...
    */
    ...
    /*悬浮函数*/
    // var  threeLeft=document.getElementsByClassName('threeLeft')[0];
    // var  threeRight=document.getElementsByClassName('threeRight')[0];
...
*
 */
function caranimateFour(obj, target, sp) {
    clearInterval(obj.timer);
    obj.timer = setInterval(function () {
        var speeds = (obj.offsetTop > target ? -sp : sp);
        if (Math.abs(obj.offsetTop - target) < sp) {
            obj.style.top = target + "px";
            clearInterval(obj.timer);
        } else {
            obj.style.top = obj.offsetTop + speeds + "px";
        }
    }, 20)
